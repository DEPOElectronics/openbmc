...
DBusMessage *reimu_s_dbus_reply = NULL;
...
{
    if (reimu_s_dbus_reply) dbus_message_unref(reimu_s_dbus_reply);
    reimu_s_dbus_reply = NULL;
}

DBusError reimu_s_dbus_error;
static int reimu_init_dbus(const char *service, const char *object, const char *interface, const char *method)
{
    ...
    reimu_dbus_fini();
    dbus_error_init(&reimu_s_dbus_error);
    if ((reimu_s_dbus_conn = dbus_bus_get(DBUS_BUS_SYSTEM, &reimu_s_dbus_error)) == NULL) return -1;

    reimu_dbus_msg_fini();
    return (reimu_s_dbus_msg = dbus_message_new_method_call(service, object, "org.freedesktop.DBus.Properties", "Set")) == NULL;
}

static int reimu_init_dbus_request(const char *string1, const char *string2)
{
    if (string2 == NULL)
        return !dbus_message_append_args(reimu_s_dbus_msg, DBUS_TYPE_STRING, &string1, DBUS_TYPE_INVALID);
    else
        return !dbus_message_append_args(reimu_s_dbus_msg, DBUS_TYPE_STRING, &string1, DBUS_TYPE_STRING, &string2, DBUS_TYPE_INVALID);
}

static int reimu_send_dbus_request(void)
{
    return !dbus_connection_send(reimu_s_dbus_conn, reimu_s_dbus_msg, NULL);
}

static int reimu_get_dbus_reply(void)
{
    reimu_dbus_reply_fini();
    return (reimu_s_dbus_reply = dbus_connection_send_with_reply_and_block(reimu_s_dbus_conn, reimu_s_dbus_msg, 1000, &reimu_s_dbus_error)) == NULL;
}
...
int reimu_dbus_get_property_bool(const char *service, const char *object, const char *interface, const char *property)
{
    ...
    DBusMessageIter dbus_iter, dbus_subiter;
    if (!dbus_message_iter_init(reimu_s_dbus_reply, &dbus_iter)) return 31;
